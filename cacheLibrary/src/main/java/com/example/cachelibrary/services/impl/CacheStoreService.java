  ...
  public Mono<CacheResponseStatus> addReactive(T object, String requestUrl, HttpHeaders headers)
    ...
    if (headers.getCacheControl() != null) {
      ...
      for (String cacheName : cacheControls) {
        ...
        strategy =
            reactiveStrategyFactory.getStrategy(ReactiveCacheControlEnum.getByCode(cacheName));
    ...
    if (headers.getETag() != null) {
      ...
      return Mono.just(
          new CacheResponseStatus(
              CacheResStatusDescripcionEnum.APLICOESTRATEGIA.getDescripcion(), HttpStatus.OK, add));
    } else {
      return null;
    }
  }
